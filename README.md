...
✅ **[Whisper](https://github.com/fedirz/faster-whisper-server)** — Локальное распознавание речи (Speech-to-Text)  
...
> 💡 **Примечание:** Минимальная конфигурация (2 CPU / 8GB RAM) подходит для работы с Gemma 3 1B (~1GB). Для комфортной работы с более крупными моделями рекомендуется 4+ CPU и 12GB RAM.

### Включенные сервисы

- ✅ **N8N** + FFmpeg — автоматизация workflows и обработка медиа
- ✅ **Ollama** — локальные LLM (Llama3, Mistral, etc.)
- ✅ **Open WebUI** — ChatGPT-подобный интерфейс
- ✅ **Supabase** — полная БД с векторным поиском (pgvector)
...
> ⚠️ **Если у вас старая версия Л.И.С.А.** — сначала обновите скрипты вручную:
> ```bash
> cd lisa
...
> python3 O6HOBA.py
> ```
> Это загрузит актуальный скрипт обновления, а затем выполнит полное обновление системы.

Скрипт обновления автоматически:
- 🔍 Определит текущую конфигурацию (GPU, окружение)
...
> 💡 **Важно:** Обновление сохраняет ваши данные и конфигурацию
...
### Автоматически устанавливаемые модели

При первом запуске системы автоматически загружаются две модели:

1. **gemma3:1b** — компактная языковая модель от Google
   - Размер: ~1 GB
   - Параметры: 1 миллиард
   - Использование: генерация текста, диалоги, ответы на вопросы
   - Особенность: высокая эффективность при минимальных ресурсах

2. **nomic-embed-text** — модель для эмбеддингов
   - Размер: ~274 MB
   - Использование: векторизация текста для RAG, семантический поиск

### Установка дополнительных моделей

После установки системы вы можете добавить любые другие модели из [библиотеки Ollama](https://ollama.com/library):

```bash
# Зайти в контейнер Ollama и установить модель
docker exec -it ollama ollama pull <model-name>

# Примеры популярных моделей:
docker exec -it ollama ollama pull llama3.2:1b      # Очень легкая (1.3 GB) - для минимальной конфигурации
docker exec -it ollama ollama pull phi3.5:mini      # Легкая (2.2 GB) - хороший баланс
docker exec -it ollama ollama pull mistral          # Средняя (4.1 GB) - качественные ответы
docker exec -it ollama ollama pull llama3.1:70b     # Большая (40 GB) - требует GPU и много RAM
...
### Управление моделями через Open WebUI

Вы также можете устанавливать, удалять и переключаться между моделями через веб-интерфейс Open WebUI:

1. Откройте http://localhost:8002
2. Перейдите в Settings → Models
3. Используйте встроенный менеджер моделей для установки новых

> 💡 **Рекомендация:** Gemma 3 1B подходит даже для минимальной конфигурации (2 CPU / 8GB RAM) благодаря компактному размеру (~1GB). Для более сложных задач можно дополнительно установить **mistral** или **llama3.1:8b**.
...
## 🔗 Интеграция Open WebUI с N8N

Вы можете интегрировать Open WebUI с N8N workflows, чтобы использовать мощь автоматизации N8N прямо из чата Open WebUI.
...
2. **Создайте N8N workflow:**
   - Откройте N8N: http://localhost:8001
   - Создайте новый workflow с Webhook нодой
   - Настройте workflow для обработки запросов
   - Скопируйте **Production webhook URL** (например: `http://localhost:8001/webhook/your-webhook-id`)

3. **Добавьте Pipe Function в Open WebUI:**
   - В Open WebUI перейдите в **Workspace → Functions → Add Function**
   - Дайте функции имя (например: "N8N Integration")
   - Добавьте описание (например: "Интеграция с N8N workflows")
   - Откройте файл `n8n_pipe.py` в корне проекта
   - Скопируйте весь код из файла и вставьте в поле кода функции

4. **Настройте параметры:**
   - Нажмите на иконку ⚙️ (шестеренка) рядом с функцией
   - Установите `n8n_url` на Production webhook URL из шага 2
   - При необходимости настройте другие параметры:
     - `n8n_bearer_token` — токен авторизации (если требуется)
     - `input_field` — поле для входных данных (по умолчанию: `chatInput`)
     - `response_field` — поле для ответа (по умолчанию: `output`)

5. **Активируйте функцию:**
   - Переключите тумблер функции в положение **ON**
   - Функция появится в выпадающем списке моделей (вверху слева в интерфейсе чата)

### Использование

После настройки вы сможете:
- Выбирать N8N функцию из списка моделей в Open WebUI
- Отправлять сообщения, которые будут обрабатываться вашими N8N workflows
- Получать ответы от N8N прямо в чате Open WebUI
- Видеть статус выполнения ("печатает...", "выполняется...")

### Примеры использования

- **RAG с векторным поиском** — используйте N8N для поиска в векторной БД и генерации ответов
- **Автоматизация задач** — запускайте сложные workflows через простой чат-интерфейс
- **Интеграция с внешними сервисами** — используйте 400+ интеграций N8N через Open WebUI

> 💡 **Совет:** Функция также опубликована на [сайте Open WebUI](https://openwebui.com/), откуда её можно импортировать напрямую.
...
# JWT секреты (сгенерировать [здесь](https://supabase.com/docs/guides/self-hosting/docker#generate-and-configure-api-keys)
JWT_SECRET=...
ANON_KEY=...
SERVICE_ROLE_KEY=...
```
...
- ✅ Транскрибация голосовых сообщений (локальный Whisper)
...
Нет времени на самостоятельную настройку? Мы внедрим ИИ-агентов в ваш бизнес:
- Интеграция с CRM (Bitrix24, AmoCRM)
- Автоматизация обработки заявок 24/7
- Настройка RAG и голосовых ассистентов

**Сайт:** [aibot.direct](https://aibot.direct)

### 👥 Сообщество PrideAIBot

Клуб для тех, кто хочет научиться работать с ИИ-агентами:
- Обучение N8N и промптингу
- Готовые шаблоны workflows
- Еженедельные уроки и поддержка

**Telegram-канал:** [@prideaibot](https://t.me/prideaibot)
**Контакт:** [@shorin_nikita](https://t.me/shorin_nikita)
