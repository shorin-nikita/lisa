...
> 💡 **Примечание:** Минимальная конфигурация (2 CPU / 8GB RAM) подходит для работы с Vikhr-Llama-3.2-1B-Instruct (<3GB). Для комфортной работы с более крупными моделями рекомендуется 4+ CPU и 12GB RAM.
...
1. **vikhr-llama-3.2-1b-instruct** — основная языковая модель, оптимизированная для русского языка
   - Размер: <3 GB
   - Параметры: 1 миллиард
   - Использование: генерация текста, диалоги, ответы на вопросы
   - Особенность: SFT на датасете GrandMaster-PRO-MAX с поддержкой CoT
...
> 💡 **Рекомендация:** Vikhr-Llama-3.2-1B-Instruct подходит даже для минимальной конфигурации (2 CPU / 8GB RAM) благодаря компактному размеру (<3GB). Для более сложных задач можно дополнительно установить **mistral** или **llama3.1:8b**.
